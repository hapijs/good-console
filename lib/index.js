...
internals.printEvent = function (event) {
    ...
    var eventTagsString;
    for (var i = 0, il = event.tags.length; i < il; ++i) {
        eventTagsString += event.tags[i] + ',';
    }
    eventTagsString = eventTagsString.slice(0,-1);

    var output = timestring + ', ' + eventTagsString + data;
    console.log(output);
};
