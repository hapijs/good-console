...
internals.GoodConsole.prototype._formatResponse = function (event, tags) {
    ...
    if (typeof event.headers === 'object' && event.headers) {
        headers = ' headers: ' + SafeStringify(event.headers);
    }

    if (typeof event.requestPayload === 'object' && event.requestPayload) {
        requestPayload = ' request payload: ' + SafeStringify(event.requestPayload);
    }
    ...
    this._printEvent({
        ...
        tags: tags,
        //instance, method, path, query, statusCode, responseTime, headers (if present), requestPayload (if present), responsePayload (if present)
        data: Util.format('%s: %s %s %s %s (%sms)%s%s%s', event.instance, method, event.path, query, statusCode, event.responseTime, headers, requestPayload, responsePayload)
    });
};
