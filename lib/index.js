...
internals.utility = {
    ...
    formatResponse(event, tags, settings) {
        ...
        // method, pid, error
        const output = `${event.source.remoteAddress} x-forwarded-for='${event.headers["x-forwarded-for"]}' ${event.instance}: ${method} ${event.path} ${query} ${statusCode} (${event.responseTime}ms)`;
