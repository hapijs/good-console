...
internals.GoodConsole._timeString = function (timestamp, format) {

    var m = Moment.utc(timestamp);
    return m.format(format);
};

internals.printEvent = function (event, timestampFormat) {

    var timestring = internals.GoodConsole._timeString(event.timestamp, timestampFormat);
...
internals.printRequest = function (event, timestampFormat) {
...
internals.GoodConsole.prototype._report = function (event, eventData) {
    ...
    if (event === 'ops') {
        ...
        internals.printRequest(eventData, this._settings.format);
    }
    else if (event === 'error') {
        internals.printEvent({
            timestamp: eventData.timestamp,
