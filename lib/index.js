...
internals.utility = {
    ...
    formatResponse(event, tags, settings) {
        ...
        const responsePayload = event.responsePayload || null;
        const requestPayload = event.requestPayload || null;
        ...
        // method, pid, error
        let output = `${event.instance}: ${method} ${event.path} ${query} ${statusCode} (${event.responseTime}ms)`;
        if (responsePayload || requestPayload) {
            output += `\n${SafeStringify(requestPayload, null, 2)} -> ${SafeStringify(responsePayload, null, 2)}`;
        }
